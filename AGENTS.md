...
- `Kernel/`: Contains the core implementation files
  ...
  - `UIResources.wl`: [MCP Apps](docs/mcp-apps.md) UI resource registry, client capability detection, and shared notebook delivery helpers (`CloudDeploy` when cloud connected, the public notebook hosting API otherwise or as a fallback, and experimental inline embedding)
